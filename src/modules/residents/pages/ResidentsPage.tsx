...
const ResidentsPage = () => {
  ...
  const [editingResident, setEditingResident] = useState<ResidentUser | null>(
    null,
  );
  ...
  const [managingContacts, setManagingContacts] = useState<ResidentUser | null>(
    null,
  );
  ...
  const { data: propertiesCatalog, loading: loadingProperties } =
    useCatalog("property");
  ...
  const handleCreate = async (data: any) => {
    try {
      ...
      if (res.success) {
        dispatch(
          showToast({
            message: "Residente agregado exitosamente",
            type: "success",
          }),
        );
      ...
      dispatch(
        showToast({
          message: error.messages?.[0] || "Error al crear residente",
          type: "error",
        }),
      );
    }
  ...
  const handleUpdate = async (data: any) => {
    ...
    try {
      ...
      if (res.success) {
        dispatch(
          showToast({ message: "Residente actualizado", type: "success" }),
        );
      ...
      dispatch(
        showToast({
          message: error.messages?.[0] || "Error al actualizar residente",
          type: "error",
        }),
      );
    }
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      dispatch(
        showToast({
          message: error.messages?.[0] || "Error al eliminar residente",
          type: "error",
        }),
      );
    }
        ...
        render: (row: ResidentUser) =>
          row.property ? (
            <div className="flex items-center gap-2">
              ...
              <span
                className="text-sm font-medium text-slate-600 truncate max-w-[150px]"
      ...
      },
      {
        ...
        render: (row: ResidentUser) => (
          <div className="text-sm text-slate-600">
            ...
            {row.residentProfile?.emergencyContact && (
              <div
                className="text-xs text-slate-400 mt-0.5 truncate max-w-[140px]"
                title={row.residentProfile.emergencyContact}
              >
                {row.residentProfile.emergencyContact}
              </div>
            )}
        ...
        actions: (row: ResidentUser) => (
          <div className="flex items-center gap-2">
            <ITButton
              ...
              className={
                row.residentProfile?.ineFrontUrl
                  ? "bg-emerald-500 hover:bg-emerald-600 border-none text-white"
                  : "border-slate-300 text-slate-400 hover:text-slate-500 hover:bg-slate-50"
              }
            ...
            <ITButton
              onClick={() => setResidentToDelete(row)}
              size="small"
              variant="ghost"
      ...
      >
        <div className="p-6">
          <p className="text-slate-700 mb-6">
            ¿Estás seguro de eliminar el expediente de{" "}
            <span className="font-bold">
              {residentToDelete?.name} {residentToDelete?.lastName}
            </span>
            ? Esta acción inhabilitará su acceso a la plataforma.
          </p>
        ...
        <div className="p-6 bg-slate-50 min-h-[300px]">
          ...
          viewingIne?.residentProfile?.ineBackUrl ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {viewingIne.residentProfile.ineFrontUrl && (
                <div>
                  <h4 className="text-xs font-bold text-slate-400 tracking-wider mb-2 uppercase">
                    INE Frontal
                  </h4>
                  <div className="rounded-xl overflow-hidden shadow border border-slate-200 hover:scale-105 transition-transform duration-300">
                    <img
                      src={viewingIne.residentProfile.ineFrontUrl}
                      alt="Frente"
              ...
              {viewingIne.residentProfile.ineBackUrl && (
                <div>
                  ...
                  <div className="rounded-xl overflow-hidden shadow border border-slate-200 hover:scale-105 transition-transform duration-300">
                    <img
                      src={viewingIne.residentProfile.ineBackUrl}
                      alt="Reverso"
                      className="w-full h-auto"
                    />
                  </div>
            ...
            <div className="flex flex-col items-center justify-center p-12 text-slate-400">
              ...
              <h3 className="text-lg font-medium text-slate-500">
                Sin identificaciones archivadas
              </h3>
              <p className="text-sm">
                Edita el expediente para subir fotografías del INE.
              </p>
            </div>
          )}
        </div>

...
const PropertiesPage = () => {
  ...
  const handleCreate = async (data: any) => {
    try {
      const res: any = await createProperty(data);
      if (res.success) {
        dispatch(
          showToast({
        ...
        dispatch(
          showToast({
            message: res.error?.messages?.[0] || "Error al crear",
            type: "error",
          }),
        );
      }
      ...
      dispatch(
        showToast({
          message: error.messages?.[0] || "Error al crear propiedad",
          type: "error",
        }),
      );
    }
  ...
  const handleUpdate = async (data: any) => {
    ...
    try {
      const res: any = await updateProperty(editingProperty.id, data);
      if (res.success) {
        dispatch(
      ...
      dispatch(
        showToast({
          message: error.messages?.[0] || "Error al actualizar propiedad",
          type: "error",
        }),
      );
    }
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res: any = await deleteProperty(propertyToDelete.id);
      if (res.success) {
        dispatch(
          showToast({ message: "Propiedad eliminada", type: "success" }),
        );
      ...
      dispatch(
        showToast({
          message: error.messages?.[0] || "Error al eliminar propiedad",
          type: "error",
        }),
      );
    }
    ...
    () => [
      ...
      },
      {
        ...
        render: (row: any) => {
          ...
          const status = statusMap[row.status?.name || "VACNT"] || {
            label: "DESCONOCIDO",
            color: "secondary",
          };
          return (
      ...
      },
      {
        ...
        actions: (row: any) => (
          <div className="flex items-center gap-2">
            ...
            <ITButton
              onClick={() => {
                if (row.latitude && row.longitude) {
                  ...
                  dispatch(
                    showToast({
                      message: "Propiedad sin coordenadas GPS",
                      type: "warning",
                    }),
                  );
                }
            ...
            <ITButton
              onClick={() => setPropertyToDelete(row)}
              size="small"
              variant="ghost"
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            Gestión de casas y departamentos (Catálogo Habitacional)
          </p>
        </div>
      ...
      >
        <div className="p-6">
          <p className="text-slate-700 mb-6">
            ...
            <span className="font-bold">{propertyToDelete?.identifier}</span>?
          </p>

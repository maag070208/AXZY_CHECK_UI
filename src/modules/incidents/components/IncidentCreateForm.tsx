...
import { createIncident, uploadIncidentFile, IncidentMediaItem } from "../services/IncidentService";
...
const IncidentCreateForm = ({ isOpen, onClose, onSuccess, typesCatalog }: IncidentCreateFormProps) => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>, fileType: 'IMAGE' | 'VIDEO') => {
    ...
    const uploaded = await uploadIncidentFile(file);
  ...
  const handleSubmit = async () => {
    if (!typeId) {
      dispatch(showToast({ message: "Selecciona el tipo de incidencia", type: "warning" }));
      return;
    }
    ...
    const validMedia: IncidentMediaItem[] = media
      .filter(m => m.uploaded)
      .map(m => m.uploaded!);
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col border border-slate-100">
        ...
        <div className="px-6 py-5 flex justify-between items-center bg-white border-b border-slate-100 flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-lg font-bold text-slate-800">Nuevo Reporte de Incidencia</h3>
          </div>
          <button
            onClick={handleClose}
        ...
        <div className="p-6 overflow-y-auto flex-1 bg-white space-y-6">
                    ...
                    <button
                      ...
                      className={`px-4 py-2.5 text-sm font-semibold rounded-xl border transition-all duration-200 ${
                        isSelected
                          ...
                          : 'bg-white border-slate-200 text-slate-600 hover:border-emerald-300 hover:text-emerald-700 hover:bg-emerald-50/50'
                      }`}
                    >
                      {type.value}
                    </button>
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              className="w-full p-4 rounded-xl border border-slate-200 bg-slate-50 text-sm text-slate-700 placeholder-slate-400 resize-none focus:outline-none focus:border-emerald-400 focus:ring-4 focus:ring-emerald-50 focus:bg-white transition-all"
            />
          </div>
        ...
        <div className="px-6 py-4 flex justify-end gap-3 border-t border-slate-100 bg-slate-50/50 flex-shrink-0">
          ...
          <ITButton
            variant="filled"
            color="primary"
            className="px-8 !rounded-xl shadow-lg shadow-emerald-200"

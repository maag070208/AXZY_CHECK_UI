...
import { FaCamera, FaMapMarkerAlt, FaPlus, FaTimes, FaUpload, FaVideo } from "react-icons/fa";
import { useDispatch } from "react-redux";
import { showToast } from "@app/core/store/toast/toast.slice";
import { createClub, uploadClubFile, ClubMediaItem } from "../services/ClubService";
...
const ClubCreateForm = ({ isOpen, onClose, onSuccess, typesCatalog }: ClubCreateFormProps) => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>, fileType: 'IMAGE' | 'VIDEO') => {
    ...
    const uploaded = await uploadClubFile(file);
  ...
  const handleSubmit = async () => {
    ...
    const validMedia: ClubMediaItem[] = media
      .filter(m => m.uploaded)
      .map(m => m.uploaded!);
    ...
    const res = await createClub({
      title: selectedType?.value || "Casa Club",
      categoryId: selectedType?.categoryId ? Number(selectedType.categoryId) : 0,
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col border border-slate-100">
        ...
        <div className="px-6 py-5 flex justify-between items-center bg-white border-b border-slate-100 flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-lg font-bold text-slate-800">Nuevo Reporte de Casa Club</h3>
          </div>
          <button
            onClick={handleClose}
        ...
        <div className="p-6 overflow-y-auto flex-1 bg-white space-y-6">
          ...
          <div>
            ...
            <div className="flex gap-3 mb-3">
              <button
                ...
                className="flex-1 h-[80px] flex flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed border-sky-200 bg-sky-50/30 hover:bg-sky-50 hover:border-sky-400 transition-all duration-200 group"
              >
                <div className="w-8 h-8 rounded-full bg-sky-100 flex items-center justify-center group-hover:bg-sky-200 transition-colors">
                  <FaCamera className="text-sky-600" size={14} />
                </div>
                <span className="text-xs font-bold text-sky-700">Subir Foto</span>
              </button>
              <button
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              className="w-full p-4 rounded-xl border border-slate-200 bg-slate-50 text-sm text-slate-700 placeholder-slate-400 resize-none focus:outline-none focus:border-sky-400 focus:ring-4 focus:ring-sky-50 focus:bg-white transition-all"
            />
          </div>
        ...
        <div className="px-6 py-4 flex justify-end gap-3 border-t border-slate-100 bg-slate-50/50 flex-shrink-0">
          ...
          <ITButton
            variant="filled"
            color="primary"
            className="px-8 !rounded-xl shadow-lg shadow-sky-200"
            onClick={handleSubmit}
            disabled={submitting || !typeId || isUploading}
          >
            {submitting ? (

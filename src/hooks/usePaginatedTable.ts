...
export function usePaginatedTable({
  ...
  const saveStateToStore = useCallback(
    (currentState: {
      pageIndex: number;
      pageSize: number;
      ...
      if (saveStateToRedux && tableKey) {
        ...
        if (
          JSON.stringify(stateToSave) !== JSON.stringify(lastSavedState.current)
        ) {
  ...
  const fetchData = useCallback(
    async (overridePageSize?: number) => {
      ...
      try {
        ...
        params.append("pageSize", String(overridePageSize ?? state.pageSize));
  ...
  const fetchDataWithOverride = useCallback(
    async (
      ...
      try {
        ...
        const sortDescendingToUse =
          overrideSortDescending !== undefined
        ...
        if (searchTermToUse?.trim())
          params.append("SearchTerm", searchTermToUse.trim());
